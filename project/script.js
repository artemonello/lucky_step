...
$(this).toggleClass('open');
  ...
  $('.search_mobile').removeClass('down');
  $('.fa fa-camera-retro fa-3x').removeClass('fa-li fa fa-check-square');
});


$('nav li a').click(function(){
    $('.menu-mobile').addClass('#menu-toggle');
   $('.menu-mobile').removeClass('fa-li fa fa-check-square');
    $('nav').toggleClass('down');
});


$('.fa fa-camera-retro').click(function(){
    $(this).toggleClass('fa-li fa fa-check-square');
   
    $('.search_mobile').toggleClass('down');
...
$('#input0').click(function() {
 if($("select#input0 :selected").val() == "rus") {
 	...
 	$('.item2').text('Стили');
 	$('.item3').text('Тренеры');
 	$('.item4').text('Рассписание');
 	$('.item5').text('Контакты');
 	$('.heading').text('О школе');
 	$('.search_field').attr('placeholder','Поиск...');
 	$('.search_button').attr('value','Искать');
 	$('.article-1 h1').text('Опыт');
 	$('.article-2 h1').text('Награды');
 }
 if($("select#input0 :selected").val() == "eng") {
 	 $('.item1').text('About Us');
 	$('.item2').text('Styles');
 	$('.item3').text('Trainers');
 	$('.item4').text('Timetable');
 	$('.item5').text('Contacts');
 	$('.heading').text('About school');
 	$('.search_field').attr('placeholder','Search...');
 	$('.search_button').attr('value','Find');
 	$('.article-1 h1').text('Experience');
 	$('.article-2 h1').text('Achivements');
 }

});
var account = document.querySelector('.account');
var account_form = document.querySelector('.account_form');
...
document.addEventListener('click', function (e) {
  var target = e.target;
  var its_account_form = target == account_form || account_form.contains(target);
  var its_account = target == account;
  var account_form_is_active = account_form.classList.contains('active');
  if (!its_account_form && !its_account && account_form_is_active) {
    toggleMenu();
    ...
    (a = function() {
      ...
      function e(a) {
        null == a && (a = {}), this.scrollCallback = f(this.scrollCallback, this),
          ...
          0, this.config = this.util().extend(a, this.defaults), null != a.scrollContainer &&
          (this.config.scrollContainer = document.querySelector(a.scrollContainer)),
          this.animationNameCache = new c, this.wowEvent = this.util().createEvent(
      ...
      return e.prototype.defaults = {
          ...
          if (this.stopped = !1, this.boxes = function() {
            ...
            if (this.disabled()) this.resetStyle();
              ...
              for (e = this.boxes, c = 0, d = e.length; d > c; c++) b = e[c], this.applyStyle(
                b, !0);
          return this.disabled() || (this.util().addEvent(this.config.scrollContainer ||
            window, "scroll", this.scrollHandler), this.util().addEvent(window,
            "resize", this.scrollHandler), this.interval = setInterval(this.scrollCallback,
            50)), this.config.live ? new a(function(a) {
            return function(b) {
              var c, d, e, f, g;
          ...
          return this.stopped = !0, this.util().removeEvent(this.config.scrollContainer ||
            window, "scroll", this.scrollHandler), this.util().removeEvent(window,
            "resize", this.scrollHandler), null != this.interval ? clearInterval(
